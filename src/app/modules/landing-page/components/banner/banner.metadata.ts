export const resume = {
    skills: [
        {
            topic: 'Frontend Web Development',
            skill: [
                {
                    path: 'assets/icons/technology/html.svg',
                    name: 'HTML'
                },
                {
                    path: 'assets/icons/technology/css.svg',
                    name: 'CSS'
                },
                {
                    path: 'assets/icons/technology/angular.png',
                    name: 'Angular2+'
                },
                {
                    path: 'assets/icons/technology/react.svg',
                    name: 'React'
                },
                {
                    path: 'assets/icons/technology/vue.svg',
                    name: 'Vue'
                },
                {
                    path: 'assets/icons/technology/polymer.svg',
                    name: 'Polymer1'
                },
                {
                    path: 'assets/icons/technology/less.svg',
                    name: 'Less'
                },
                {
                    path: 'assets/icons/technology/scss.svg',
                    name: 'Sass / Scss'
                },
                {
                    path: 'assets/icons/technology/typescript.svg',
                    name: 'Typescript'
                },
                {
                    path: 'assets/icons/technology/redux.svg',
                    name: 'Redux'
                },
                {
                    path: 'assets/icons/technology/rxjs.svg',
                    name: 'RxJS'
                }
            ]
        },
        {
            topic: 'Backend Web Development',
            skill: [
                {
                    path: 'assets/icons/technology/php.svg',
                    name: 'PHP'
                },
                {
                    path: 'assets/icons/technology/nodejs.svg',
                    name: 'NodeJS'
                },
                {
                    path: 'assets/icons/technology/express.svg',
                    name: 'Express'
                }
            ]
        },
        {
            topic: 'DB Web Development',
            skill: [
                {
                    path: 'assets/icons/technology/mysql.svg',
                    name: 'MySQL'
                },
                {
                    path: 'assets/icons/technology/prosgresql.svg',
                    name: 'ProsgresQL'
                },
                {
                    path: 'assets/icons/technology/mongodb.svg',
                    name: 'MongoDB'
                },
                {
                    path: 'assets/icons/technology/firebase.svg',
                    name: 'Firebase'
                }
            ]
        },
        {
            topic: 'Testing Tools',
            skill: [
                {
                    path: 'assets/icons/technology/karma.svg',
                    name: 'Karma'
                },
                {
                    path: 'assets/icons/technology/jasmine.svg',
                    name: 'Jasmine'
                }
            ]
        },
        {
            topic: 'Utilization Tools',
            skill: [
                {
                    path: 'assets/icons/technology/webpack.svg',
                    name: 'Webpack'
                },
                {
                    path: 'assets/icons/technology/docker.svg',
                    name: 'Docker'
                },
                {
                    path: 'assets/icons/technology/postman.svg',
                    name: 'Postman'
                }
            ]
        },
        {
            topic: 'Other Programming Language',
            skill: [
                {
                    path: 'assets/icons/technology/java.svg',
                    name: 'Java'
                },
                {
                    path: 'assets/icons/technology/python.svg',
                    name: 'Python'
                }
            ]
        },
        {
            topic: 'Collaboration Tools',
            skill: [
                {
                    path: 'assets/icons/technology/git.svg',
                    name: 'Git'
                },
                {
                    path: 'assets/icons/technology/confluence.svg',
                    name: 'Confluence'
                },
                {
                    path: 'assets/icons/technology/jira.svg',
                    name: 'Jira'
                },
                {
                    path: 'assets/icons/technology/team.svg',
                    name: 'Microsoft team'
                },
                {
                    path: 'assets/icons/technology/slack.svg',
                    name: 'Slack'
                }
            ]
        }
    ],
    mainSection: [
        {
            sectionName: 'EDUCATION',
            details: [
                {
                    name: 'Chulalongkorn University',
                    dateTime: 'Aug 2015 - May 2019',
                    descriptions: [
                        {
                            selector: 'item',
                            label: '<b>GPAX:</b> 3.06'
                        },
                        {
                            selector: 'item',
                            label: '<b>Elective Courses:</b>Data Science and Engineering, Artificial intelligence, Cloud computing (AWS), Computer Security, MERN Stack, High Tech Entrepreneurship'
                        }
                    ]
                }
            ]
        },
        {
            sectionName: 'WORK EXPERIENCE',
            details: [
                {
                    name: 'Refinitiv Software (Thailand)',
                    dateTime: 'June 2019 - Present',
                    descriptions: [
                        {
                            selector: 'label',
                            label: 'Descriptions'
                        },
                        {
                            selector: 'item',
                            label: 'Develop new user-facing features'
                        },
                        {
                            selector: 'item',
                            label: 'Build reusable code and libraries for future use'
                        },
                        {
                            selector: 'item',
                            label: 'Ensure the technical feasibility of UI/UX designs'
                        },
                        {
                            selector: 'item',
                            label: 'Optimize application for maximum speed and scalability'
                        },
                        {
                            selector: 'item',
                            label: 'Assure that all user input is validated before submitting to backend'
                        },
                        {
                            selector: 'item',
                            label: 'Collaborate with other team members and stakeholders'
                        },
                        {
                            selector: 'label',
                            label: 'Tools'
                        },
                        {
                            selector: 'item',
                            label: 'HTML, Css, Javascript'
                        },
                        {
                            selector: 'item',
                            label: 'Angular, Typescript, Rxjs'
                        },
                        {
                            selector: 'item',
                            label: 'Polymer1'
                        },
                        {
                            selector: 'item',
                            label: 'Vanilla Js'
                        },
                        {
                            selector: 'item',
                            label: 'Typescript'
                        },
                        {
                            selector: 'item',
                            label: 'Web worker'
                        },
                        {
                            selector: 'item',
                            label: 'Karma, Jasmine (Testing tools)'
                        },
                        {
                            selector: 'item',
                            label: 'Git (Version control)'
                        },
                        {
                            selector: 'item',
                            label: 'Jira, Confluence, Microsoft team (Collaboration tools)'
                        }
                    ]
                },
                {
                    name: 'Bank of Ayudhya (Krungsri)',
                    dateTime: 'June 2018 - July 2018',
                    descriptions: [
                        {
                            selector: 'label',
                            label: 'Descriptions'
                        },
                        {
                            selector: 'item',
                            label: 'Developed NDID project'
                        },
                        {
                            selector: 'item',
                            label: 'Implement automated test using Robot Framework'
                        },
                        {
                            selector: 'label',
                            label: 'Tools'
                        },
                        {
                            selector: 'item',
                            label: 'Go'
                        },
                        {
                            selector: 'item',
                            label: 'Robot Framework'
                        },
                        {
                            selector: 'item',
                            label: 'Tendermint'
                        },
                        {
                            selector: 'item',
                            label: 'Docker'
                        },
                        {
                            selector: 'item',
                            label: 'Postman'
                        },
                        {
                            selector: 'item',
                            label: 'Slack'
                        }
                    ]
                }
            ]
        },
        {
            sectionName: 'PROJECTS',
            details: [
                {
                    name: 'AQI and PM2.5 one-page Generator for Eastern Thailand',
                    descriptions: [
                        {
                            selector: 'label',
                            label: 'Descriptions'
                        },
                        {
                            selector: 'item',
                            label: 'Displayed AQI and PM2.5 details for Eastern Thailand'
                        },
                        {
                            selector: 'item',
                            label: 'Added the ability to save as the image'
                        },
                        {
                            selector: 'item',
                            label: 'Added the ability to send the image to LINE via LINE Notify'
                        },
                        {
                            selector: 'label',
                            label: 'Tools'
                        },
                        {
                            selector: 'item',
                            label: 'Angular2+ including scss, typescript, rxjs'
                        },
                        {
                            selector: 'item',
                            label: 'NodeJS including express'
                        },
                        {
                            selector: 'item',
                            label: 'Firebase (using for storing the data)'
                        },
                        {
                            selector: 'item',
                            label: 'Puppeteer (using for generating the screenshot from backend side'
                        },
                        {
                            selector: 'item',
                            label: 'Cronjobs (using for setting interval to generate the image)'
                        },
                        {
                            selector: 'item',
                            label: 'LINE Notify (using for sending the image to LINE)'
                        }
                    ]
                },
                {
                    name: 'Play OK generator',
                    descriptions: [
                        {
                            selector: 'label',
                            label: 'Descriptions'
                        },
                        {
                            selector: 'item',
                            label: 'Convert Thaichess record from English to Thai'
                        },
                        {
                            selector: 'label',
                            label: 'Tools'
                        },
                        {
                            selector: 'item',
                            label: 'Vue'
                        }
                    ]
                },
                {
                    name: 'Book Recommendation System For Library',
                    descriptions: [
                        {
                            selector: 'label',
                            label: 'Descriptions'
                        },
                        {
                            selector: 'item',
                            label: 'Using machine learning (Collabolartive Filtering) to train the data'
                        },
                        {
                            selector: 'item',
                            label: 'Grouping the data and Recommend by faculty'
                        },
                        {
                            selector: 'item',
                            label: 'using NLP to recommend by the name of title'
                        },
                        {
                            selector: 'label',
                            label: 'Tools'
                        },
                        {
                            selector: 'item',
                            label: 'React'
                        },
                        {
                            selector: 'item',
                            label: 'NodeJS'
                        },
                        {
                            selector: 'item',
                            label: 'MongoDB'
                        },
                        {
                            selector: 'item',
                            label: 'ProsgresQL'
                        },
                        {
                            selector: 'item',
                            label: 'Python (scikit-learn, pandas)'
                        }
                    ]
                }
            ]
        }
    ]
}